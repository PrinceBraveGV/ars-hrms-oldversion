...
namespace AristaHRM.Controllers
{
    ...
    public class NotifController : Controller
    {
        ...
        {
            return RedirectToAction("MutasiAtasan", "Admin");
        }
        ...
        {
            ViewData["ErrorData"] = (Session["ErrorData"] ?? string.Empty).ToString();
            Session["ErrorData"] = null;
            return RedirectToAction("Index", "Home");
        }
        ...
        {
            Status_Cuti = Session["Status_Cuti"]?.ToString() ?? string.Empty;
            if (!string.IsNullOrWhiteSpace(Status_Cuti) && Status_Cuti.Equals("Disetujui"))
            {
                return RedirectToAction("ApprovalSukses", "Notif");
            }
            else if (!string.IsNullOrWhiteSpace(Status_Cuti) && Status_Cuti.Equals("Ditolak"))
            {
                return RedirectToAction("ApprovalReject", "Notif");
            }
            else
            {
                return View("ApprovalError");
            }
        }
        ...
        public ActionResult ApprovalSukses()
        {
            ...
            string privilege = Session["Privilege"] as string;
            string jenis = Session["Jenis_Cuti"] as string;
            ...
            switch (privilege)
            {
                case "Admin":
                    if (jenis == "Khusus") 
                    {
                        ViewBag.RedirectUrl = Url.Action("ApprovalKhusus", "Admin");
                    }
                    else
                    {
                        ViewBag.RedirectUrl = Url.Action("ApprovalTahunan", "Admin");
                    }
                    break;
                case "Manager":
                    if (jenis == "Khusus")
                    {
                        ViewBag.RedirectUrl = Url.Action("ApprovalKhusus", "Manager");
                    }
                    else
                    {
                        ViewBag.RedirectUrl = Url.Action("ApprovalTahunan", "Manager");
                    }
                    break;
                case "Supervisor":
                    if (jenis == "Khusus")
                    {
                        ViewBag.RedirectUrl = Url.Action("ApprovalKhusus", "Supervisor");
                    }
                    else
                    {
                        ViewBag.RedirectUrl = Url.Action("ApprovalTahunan", "Supervisor");
                    }
                    break;
            }
        ...
        public ActionResult ApprovalReject(CutiModel model)
        {
            ...
            if (!string.IsNullOrEmpty(model.Alasan))
            {
                ...
                {
                    NamaProsedur = "SP_ApproveKhusus";
                }
                else
                {
                    NamaProsedur = "SP_ApproveTahunan";
                }
                ...
                parameters.Add(new SqlParameter("@ID_Cuti", ID_Cuti));
                parameters.Add(new SqlParameter("@Alasan", Alasan));
                parameters.Add(new SqlParameter("@Selector", "Alasan"));
                ...
                switch (privilege)
                {
                    case "Admin":
                        if (jenis == "Khusus")
                        {
                            redirectUrl = Url.Action("ApprovalKhusus", "Admin");
                        }
                        else
                        {
                            redirectUrl = Url.Action("ApprovalTahunan", "Admin");
                        }
                        break;
                    case "Manager":
                        if (jenis == "Khusus")
                        {
                            redirectUrl = Url.Action("ApprovalKhusus", "Manager");
                        }
                        else
                        {
                            redirectUrl = Url.Action("ApprovalTahunan", "Manager");
                        }
                        break;
                    case "Supervisor":
                        if (jenis == "Khusus")
                        {
                            redirectUrl = Url.Action("ApprovalKhusus", "Supervisor");
                        }
                        else
                        {
                            redirectUrl = Url.Action("ApprovalTahunan", "Supervisor");
                        }
                        break;
                }
                ...
                {
                    cmd = new SqlCommand(NamaProsedur, sqlconn);
                ...
                //try
                //{
                //    DB.Database.ExecuteSqlCommand(Helpers.GenerateCommandText(NamaProsedur, parameters.ToArray()), parameters.ToArray());
                //}
                //catch (SqlException ex)
                //{
                //    // jika terdapat gangguan koneksi database
