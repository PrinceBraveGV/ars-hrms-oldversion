    ...
    public class UserController : Controller
    {
        ...
        public ActionResult Profil(KaryawanModel model, string id, string Length)
        {
            ...
            else
            {
                ...
                NIK = model.NIK.ToString().Trim(); NIK = model.NIK.ToString().Trim();
                id = NIK;
        ...
        // kendali khusus untuk refresh halaman
            ...
            using (var conn = new SqlConnection(connstring))
            {
                ...
                try
                {
                    ...
                    if (string.IsNullOrEmpty(model.Keperluan) && model.Jenis_Cuti == "Cuti Khusus")
                        ViewData["ErrorMsg"] = "Keperluan cuti khusus yang bersangkutan harus diisi.";
        ...
        public ActionResult Pengajuan(CutiModel model, string returnUrl)
        {
            ...
            using (var conn = new SqlConnection(connstring))
            {
                ...
                {
                    ViewData["ErrorMsg"] = "Mohon menghubungi HR department untuk melengkapi data anda sebelum dapat melakukan pengajuan cuti.";
                    model.NIK = NIK;
                    SetPrivilege();
                    return View(model);
                }
                ...
                if (!string.IsNullOrEmpty(model.Nama_Karyawan) && !string.IsNullOrEmpty(model.Jenis_Cuti) && !string.IsNullOrEmpty(model.Keperluan))
                {
                    ...
                    catch (SqlException ex)
                    {
                        ...
                        ViewData["ErrorMsg"] = "Terjadi kegagalan koneksi database saat memproses data. Pesan: \"" + ex.Message + "\"";
                        SetPrivilege();
                        CreateErrorLog(ex.Message, (ex.InnerException != null ? ex.InnerException.ToString() : string.Empty), ex.StackTrace);
                        return View(model);
                    }
                    catch (Exception e)
                    {
                        ...
                        ViewData["ErrorMsg"] = "Terjadi masalah tak dikenal dalam proses pengisian data. Segera hubungi admin/support dengan pesan berikut ini: \"" + e.Message + "\"";
                        SetPrivilege();
                        CreateErrorLog(e.Message, (e.InnerException != null ? e.InnerException.ToString() : string.Empty), e.StackTrace);
                        return View(model);
                    }
                    finally
                    {
            ...
            if (!string.IsNullOrEmpty(model.NIK) && !string.IsNullOrEmpty(model.Tgl_Resign.ToString()) && !string.IsNullOrEmpty(model.Alasan))
            {
                ...
                using (var conn = new SqlConnection(connstring))
                {
                    ...
                    try
                    {
                        ...
                        return RedirectToAction("ResignSukses", "Notif");
                    }
                    catch (SqlException ex)
                    {
                        // jika terdapat gangguan koneksi database
        ...
        public ActionResult RememberAtasan()
        {
            ...
            return RedirectToAction("Riwayat", "User");
        }
        #endregion
        ...
        public void CreateErrorLog(string message, string innerException, string stackTrace)
        {
            string dateFormat = DateTime.Now.ToShortDateString().ToString() + " " + DateTime.Now.ToLongTimeString().ToString() + " ==> ";
            string timeStamp = string.Format("{0}{1}{2}", DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Date);
            string path = Path.GetPathRoot(Environment.CurrentDirectory);
            string combinedPath = Path.Combine(path, "ErrorLog.txt");

            using (var sw = new StreamWriter(combinedPath, true))
            {
                sw.WriteLine(dateFormat + " - " + message + " - " + innerException + " - " + stackTrace);
                sw.Flush();
                sw.Close();
            }
        }
