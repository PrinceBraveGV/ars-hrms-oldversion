...
namespace AristaHRM.Controllers
{
    ...
    public class InputController : Controller
    {
        ...
        public string NIK_Atasan, NIK_Supervisor;
        public string Nama_Atasan, Nama_Supervisor;
        public string NIK_Advisor, Nama_Advisor;
        public string Pembuat, Privilege, Status_Karyawan, Status_Kerja, Area_Kerja;

        // administrasi pajak dan BPJS
        public string NPWP, Status_WP, NoBPJSKS, NoBPJSTK;
        ...
        #region Perusahaan
        [AllowAnonymous]
        public ActionResult AtasanTRPartial()
        {
            ...
            return PartialView("_AtasanTR", new KaryawanModel { Perusahaan = Perusahaan, Cabang = Cabang, Departemen = Departemen });
        }

        [AllowAnonymous]
        public ActionResult SupervisorTRPartial()
        {
            string Perusahaan = (Request.Params["Perusahaan"] ?? string.Empty).ToString();
            string Cabang = (Request.Params["Cabang"] ?? string.Empty).ToString();
            string Departemen = (Request.Params["Departemen"] ?? string.Empty).ToString();
            return PartialView("_SupervisorTR", new KaryawanModel { Perusahaan = Perusahaan, Cabang = Cabang, Departemen = Departemen });
        }
        ...
        public ActionResult InputKaryawan()
        {
            ...
            else
            {
                ...
                {
                    DateTime TglLahir = DateTime.Now.AddYears(-18);
                    ...
                    return View(new KaryawanModel { Perusahaan = Nama_Perusahaan, Cabang = Nama_Cabang, Tgl_Lahir = TglLahir });
                }
                else
                {
        ...
        {
            string Nama_Perusahaan = (Request.Params["Perusahaan"] ?? string.Empty).ToString();
            return PartialView(new KaryawanModel { Perusahaan = Nama_Perusahaan });
        }
        ...
        public ActionResult StatusWPPartial()
        {
            string Status_Nikah = (Request.Params["Status_Nikah"] ?? string.Empty).ToString();
            return PartialView("_StatusWP", new KaryawanModel { Status_Nikah = Status_Nikah });
        }
        ...
        {
            // update 190729:
            // NIK diproduksi secara otomatis sesuai dengan nama perusahaan, departemen dan urutan masuk pada perusahaan & departemen yang sama
            if (ModelState.IsValid)
            {
                ...
                if (urutan == null)
                {
                    ...
                    var angka = 1;
                    model.NIK_Temp = prefix + angka.ToString().PadLeft(5, '0');
                }
                else
                {
                ...
                using (var conn = new SqlConnection(connstring))
                {
                    ...
                    */
                    ...
                    // masalah terjadi apabila NIK kosong - harus dibuat NIK auto-generated berdasarkan informasi dari HR dept.
                ...
                //{
                    ...
                    Alamat = model.Alamat;
                    Agama = model.Agama;
                    Status_Nikah = model.Status_Nikah;
                    Email = model.Email;
                    ...
                    NIK_Atasan = model.NIK_Atasan.ToString().Trim();
                    Nama_Atasan = DB.TM_Karyawans.FirstOrDefault(x => x.NIK == NIK_Atasan).Nama_Karyawan;
                    // Nama_Atasan = model.Nama_Atasan.ToString().Trim();
                    if (!string.IsNullOrWhiteSpace(model.NIK_Supervisor) || !string.IsNullOrWhiteSpace(model.Nama_Supervisor))
                    {
                        NIK_Supervisor = model.NIK_Supervisor.ToString().Trim();
                        Nama_Supervisor = DB.TM_Karyawans.FirstOrDefault(x => x.NIK == NIK_Supervisor).Nama_Karyawan;
                    }

                    if (!string.IsNullOrWhiteSpace(model.NIK_Advisor))
                    {
                        NIK_Advisor = model.NIK_Advisor.ToString().Trim();
                        Nama_Advisor = DB.TM_Karyawans.FirstOrDefault(x => x.NIK == NIK_Advisor).Nama_Karyawan;
                    }
                    Status_Karyawan = model.Status_Karyawan.ToString().Trim();
                    ...
                    NPWP = model.NPWP;
                    Status_WP = model.Status_WP;
                    NoBPJSKS = model.NoBPJSKS;
                    NoBPJSTK = model.NoBPJSTK;
                    ...
                    {
                        cmd = new SqlCommand("SM_Karyawan", connection);
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add("@NIK", SqlDbType.NVarChar).Value = NIK;
                        cmd.Parameters.Add("@NIK_Temp", SqlDbType.NVarChar).Value = model.NIK_Temp;
                        cmd.Parameters.Add("@Nama_Karyawan", SqlDbType.NVarChar).Value = Nama_Karyawan;
                        ...
                        if (!string.IsNullOrWhiteSpace(Email_Perusahaan))
                        {
                        ...
                        cmd.Parameters.Add("@NIK_Atasan", SqlDbType.NVarChar).Value = NIK_Atasan;
                        cmd.Parameters.Add("@Nama_Atasan", SqlDbType.NVarChar).Value = Nama_Atasan;

                        if (!string.IsNullOrWhiteSpace(NIK_Supervisor) || !string.IsNullOrWhiteSpace(Nama_Supervisor))
                        {
                            cmd.Parameters.Add("@NIK_Supervisor", SqlDbType.NVarChar).Value = NIK_Supervisor;
                            cmd.Parameters.Add("@Nama_Supervisor", SqlDbType.NVarChar).Value = Nama_Supervisor;
                        }

                        if (!string.IsNullOrWhiteSpace(NIK_Advisor))
                        {
                            cmd.Parameters.Add("@NIK_Advisor", SqlDbType.NVarChar).Value = NIK_Advisor;
                        ...
                        cmd.Parameters.Add("@NPWP", SqlDbType.NVarChar).Value = NPWP;
                        cmd.Parameters.Add("@Status_WP", SqlDbType.NVarChar).Value = Status_WP;
                        cmd.Parameters.Add("@NoBPJSKS", SqlDbType.NVarChar).Value = NoBPJSKS;
                        cmd.Parameters.Add("@NoBPJSTK", SqlDbType.NVarChar).Value = NoBPJSTK;
                        ...
                        cmd.Parameters.Add("@ModifiedBy", SqlDbType.NVarChar).Value = ModifiedBy;
        ...
        #region Input Data Karyawan Trainee/Percobaan
        public ActionResult InputKaryawanTrainee()
        {
            if (!Request.IsAuthenticated)
            {
                return RedirectToAction("UserSession", "Home", new { ReturnUrl = Request.Url.PathAndQuery.ToString() });
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult InputKaryawanTrainee(KaryawanModel model)
        {
            // update 190729:
            // NIK diproduksi otomatis dari sistem dengan pola sbb:
            // [kode singkatan perusahaan] - [kode singkatan departemen] - [nomor urut terakhir] 
            if (ModelState.IsValid)
            {
                // tahap 1: transfer model ke variabel kendali
                Nama_Karyawan = model.Nama_Karyawan.ToString().Trim();
                ...
                var kodePerusahaan = DB.TM_Perusahaans.Where(x => x.Nama_Perusahaan == model.Perusahaan).Select(x => x.Kode_Singkat).FirstOrDefault();
                var kodeDept = DB.TM_Departemens.Where(x => x.Nama_Dept == model.Departemen).Select(x => x.Singkatan).FirstOrDefault();
                ...
                using (var conn = new SqlConnection(connstring))
                {
                    ...
                    //cmd.CommandType = CommandType.StoredProcedure;
                    //cmd.Parameters.Add("@Parameter", SqlDbType.NVarChar).Value = Nama_Karyawan;
                    //cmd.Parameters.Add("@Selector", SqlDbType.NVarChar).Value = "Trainee";
        ...
        public ActionResult InputCabang(CabangModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                Nama_Cabang = model.Nama_Cabang;
                Kode_Singkat = model.Kode_Singkat;
                Lokasi = model.Lokasi;
                Kepala_Cabang = model.Kepala_Cabang;
        ...
        {
            var model = DB.TM_List_Cutis.OrderByDescending(x => x.Tgl_Cuti);
            return PartialView("_CutiMassalPusat", model.ToList());
        }
        ...
        public ActionResult UpdateCM(TM_List_Cuti items)
        {
            ...
            {
                return PartialView("_CutiMassalCabang", model.ToList());
            }
            else
            {
                return PartialView("_CutiMassalPusat", model.ToList());
        ...
        {
            var model = DB.TM_List_Liburs.OrderByDescending(x => x.Tgl_Libur);
            return PartialView("_Liburan", model.ToList());
        }
