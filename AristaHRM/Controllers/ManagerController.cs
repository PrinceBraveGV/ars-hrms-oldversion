...
namespace AristaHRM.Controllers
{
    ...
    public class ManagerController : Controller
    {
        ...
        public string Nama_Perusahaan, Nama_Cabang, Jabatan, Departemen, NIK_Atasan, Nama_Atasan, NIK_Supervisor, Nama_Supervisor, Nama_Advisor;
        ...
        // jika manajer mengirim permintaan tampilan profil dengan NIK tertentu
        [HttpPost]
        ...
        // Proses submit untuk edit profil karyawan yang dipilih
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditProfil(KaryawanModel model)
        {
            ...
            {
                ViewData["ErrorMsg"] = "Proses edit tidak bisa dilakukan karena ada bagian penting yang belum diisi. Silakan periksa kembali isian anda.";
                return View(model);
            }
        }
        #endregion
        ...
        public ActionResult ApprovalKhusus(String id = null)
        {
            if (!Request.IsAuthenticated)
            {
                return RedirectToAction("UserSession", "Home", new { ReturnUrl = Request.Url.PathAndQuery.ToString() });
        ...
        public ActionResult SetujuCK(string id)
        {
            ...
            var model = (from Approval in DB.TT_Approval_K
                         where (Approval.Pemberi == Nama_Atasan)
                         select Approval).ToList();
        ...
        [HttpGet]
        public ActionResult ApprovalTahunan()
        {
            if (!Request.IsAuthenticated)
            {
                return RedirectToAction("UserSession", "Home", new { ReturnUrl = Request.Url.PathAndQuery.ToString() });
        ...
        public ActionResult SetujuMSCT(String[] arg, CutiModel model)
        {
            ...
            foreach (String idc in arg)
            {
                ...
                try
                {
                    // untuk meringankan beban email server, bagian ini dinonaktifkan jika jumlah entri melebihi batas tertentu
                    if (arg.Length <= 3)
                    {
                        Helpers.SendPersetujuan(Msg.ToString(), Email_Temp, Email_Atasan, Email_Perusahaan, Email);
                    }
            ...
            using (var conn = new SqlConnection(connstring))
            {
                ...
                try
                {
                    ...
                    while (sdr.Read())
                    {
                        ...
                        Email_Perusahaan = sdr["Email_Perusahaan"].ToString().Trim();
                        Nama_Atasan = sdr["Nama_Atasan"].ToString().Trim();
                        Nama_Supervisor = sdr["Nama_Supervisor"] != DBNull.Value ? sdr["Nama_Supervisor"].ToString() : string.Empty;
                        Privilege = sdr["Privilege"].ToString().Trim();
                    }
                }
                catch (SqlException ex)
        ...
        public ActionResult TolakMSCK(String[] arg, String res, CutiModel model)
        {
            ...
            foreach (String idc in arg)
            {
                ...
                catch (SmtpException smtpex)
                {
                    ...
                    HttpContext.Response.AddHeader("FAILED_LOAD", "1");
                    Response.StatusCode = 401;
                    // return RedirectToAction("MultiApprovalError", "Notif");
                    // return Redirect(DevExpressHelper.GetUrl(new { Controller = "Notif", Action = "MultiApprovalError" }));
                }
                catch (Exception e)
                {
        ...
        public ActionResult TolakMSCT(String[] arg, String res, CutiModel model)
        {
            ...
            Email_Supervisor = ((from Kar in DB.TM_Karyawans
                                 where Kar.Nama_Supervisor == Nama_Supervisor
                                 select Kar.Nama_Supervisor) ?? (from Kar in DB.TM_Karyawans
                                                                 where Kar.Nama_Supervisor == Email_Supervisor
                                                              select Kar.Email)).ToString().Trim() ?? string.Empty;
                    ...
                    if (!string.IsNullOrEmpty(id))
                    {
                        ...
                        using (var conn = new SqlConnection(connstring))
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    ViewBag.Message = smtpex.Message;
                                    return RedirectToAction("BatalCutiError", "Notif");
                                }
                                catch (Exception e)
                                {
                                    ViewBag.Message = e.Message;
                        ...
                        using (var conn = new SqlConnection(connstring))
                        {
                            ...
                            try
                            {
                                ...
                                try
                                {
                                    ...
                                    if (ViewBag.Privilege == "Manager")
                                    {
                                        Msg.Append("Cuti tahunan anda dengan nomor ID # " + ID_Cuti + " telah dibatalkan.");
                                    }
                                    else if (ViewBag.Privilege == "Supervisor")
                                    {
                                        Msg.Append("Cuti tahunan anda dengan nomor ID # " + ID_Cuti + " telah dibatalkan.");
        ...
        {
            var model = DB.TM_Riwayats.Where(x => x.Status_Cuti == "Tunggu" && x.Pemberi == User.Identity.Name);
